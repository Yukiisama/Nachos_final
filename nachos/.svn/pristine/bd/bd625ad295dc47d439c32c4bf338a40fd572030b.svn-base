...
ExceptionHandler (ExceptionType which)
{
	...
	case SyscallException:
		...
		case SC_Exit:
		{
			...
			//printf("\n[CAS EXIT] : Nb process : %d , Nb threads : %d \n",getNbProcess(),currentThread->space->GetNbThread());
			...
			{
				//printf("Cas où le fork ne contient aucun thread et donc n'appeleras pas ThreadExit\n");
				decrementProcess();
		...
		/*********************Bonus Exit where thread of a process calls Exit ( BonusExit in our case
		 * Cause we need to differenciate the usual exit from this special case) *********************/
		case SC_BonusExit:
		{
			//currentThread->space
			currentThread->space->bonusExit = true;
			break;
		}
